...
describe('The Section route and controller', () => {
    ...
    it('creates one section through post request with courseId', (done) => {
        request(app).post('/api/section/createSection').send({ session: dummy_reload.SESSION_ADMIN, classId: 1, title: 'Test', subtitle: "Test", ordering: 1 }).end(
            (err, response) => {
                console.log(response.body)
                assert(response.body.Section.dataValues.title === "Test")
                done()
            }
        )
    })
    ...
    it('creates one course through post request without courseId with prerequisite sets', (done) => {
        request(app).post('/api/course').send({
            session: dummy_reload.SESSION_ADMIN,
            "title": "Econs",
            "description": "Course on Econs",
            "active": true,
            "prerequisite": [
                [1, 2],
                [3, 4]
