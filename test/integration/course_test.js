...
describe('The courses route and controller', () => {
    ...
    it('creates one course through post request without courseId with prerequisite sets', (done) => {
        request(app).post('/api/course').send({ session: dummy_reload.SESSION_ADMIN, "title": "Econs", "description": "Course on Econs", "active": true, "prerequisite": [
                [1, 2],
                [3, 4]
            ] }).end(
            (err, response) => {
    ...
    it('creates one course through post request with courseId with prerequisite sets', (done) => {
        request(app).post('/api/course').send({ session: dummy_reload.SESSION_ADMIN, "courseId": 8, "title": "Econs", "description": "Course on Econs", "active": true, "prerequisite": [
                [1, 2],
                [3, 4]
            ] }).end(
            (err, response) => {
